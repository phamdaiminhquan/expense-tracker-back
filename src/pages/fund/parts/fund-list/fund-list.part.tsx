        ...
        <header className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-widest font-semibold mb-1">XIN CHÀO</p>
                <p className="font-bold text-xl text-foreground">{currentUserName}</p>
              </div>
            </div>
            <Button
              variant="ghost"
        ...
        <div className="space-y-5">
          ...
          {isLoading ? (
            ...
            <Card className="p-16 text-center border-2 border-dashed border-border/40 space-y-5 bg-card/40 backdrop-blur-xl shadow-lg">
              <div className="space-y-2">
                <p className="text-sm font-semibold text-foreground">Chưa có quỹ nào</p>
                <p className="text-xs text-muted-foreground">Tạo quỹ đầu tiên để bắt đầu!</p>
              </div>
              {onRefresh && (
                <Button variant="outline" size="sm" onClick={onRefresh} className="shadow-md">
                  Tải lại
                </Button>
              )}
            </Card>
                    ...
                    <div className="flex items-center gap-5 flex-1">
                      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary/25 to-primary/15 flex items-center justify-center shrink-0 shadow-lg ring-2 ring-primary/15 group-hover:scale-110 group-hover:ring-primary/30 transition-all duration-300">
                        {fund.type === 'shared' ? (
                          ...
                          <Wallet size={28} className="text-primary" weight="duotone" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-lg font-bold text-foreground mb-1.5 group-hover:text-primary transition-colors">{fund.name}</h3>
