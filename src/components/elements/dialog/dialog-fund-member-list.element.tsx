...
import { X, Users, User, Shield, Mail, UserPlus, Crown } from "lucide-react";
...
export function DialogFundMemberList({
  isOpen,
  onClose,
  fund,
  members = [],
  ...
  const handleRemoveMember = (e: React.MouseEvent, memberId: string) => {
    ...
    if (
      onRemoveMember &&
      window.confirm("Bạn có chắc chắn muốn xóa thành viên này?")
    ) {
            ...
            <div className="space-y-2 py-1">
              {filteredMembers.map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className={`flex max-sm:flex-col sm:items-center gap-3 p-3 rounded-lg ${
